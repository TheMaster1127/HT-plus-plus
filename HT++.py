...
for A_Index18, A_LoopField18 in enumerate(items, start=1):
    ...
    if (SubStr(Trim(StrLower(variables['A_LoopField18'])), 1 , 10)== "msgbox, % "):
        ...
        variables['lineDone'] = 1
    elif (SubStr(Trim(StrLower(variables['A_LoopField18'])), 1 , 15)== "filecreatedir, "):
        variables['FileCreateDirCommand'] = StringTrimLeft(variables['A_LoopField18'], 15)
        variables['FileCreateDirCommand2path'] = StrSplit(variables['FileCreateDirCommand'] , ", " , 1)
        variables['FileCreateDirCommand2path'] = StrReplace(variables['FileCreateDirCommand2path'] , "\\" , "\\\\")
        if ( not (InStr(variables['FileCreateDirCommand2path'] , "%"))):
            variables['FileCreateDirCommand2path'] = Trim(transpileLowVariables(variables['FileCreateDirCommand2path']))
        else:
            variables['FileCreateDirCommand2path'] = StrReplace(variables['FileCreateDirCommand2path'] , "%" , "")
        variables['cppCode'] += "FileCreateDir(" + variables['FileCreateDirCommand2path'] + ");\n"
        variables['lineDone'] = 1
    elif (SubStr(Trim(StrLower(variables['A_LoopField18'])), 1 , 15)== "fileremovedir, "):
        variables['FileRemoveDirCommand'] = StringTrimLeft(variables['A_LoopField18'], 15)
        variables['FileRemoveDirCommand2path'] = StrSplit(variables['FileRemoveDirCommand'] , ", " , 1)
        variables['FileRemoveDirCommand2path'] = StrReplace(variables['FileRemoveDirCommand2path'] , "\\" , "\\\\")
        if ( not (InStr(variables['FileRemoveDirCommand2path'] , "%"))):
            variables['FileRemoveDirCommand2path'] = Trim(transpileLowVariables(variables['FileRemoveDirCommand2path']))
        else:
            variables['FileRemoveDirCommand2path'] = StrReplace(variables['FileRemoveDirCommand2path'] , "%" , "")
        variables['cppCode'] += "FileRemoveDir(" + variables['FileRemoveDirCommand2path'] + ");\n"
        variables['lineDone'] = 1
...
for A_Index26 in range(1, variables['theIdNumOfThe34'] + 1):
    ...
    variables['cppCode'] = StrReplace(variables['cppCode'] , "ihuiuuhuuhtheidFor--asas-theuhturtyphoutr-" + Chr(65) + Chr(65) + str(variables['A_Index26']) + Chr(65) + Chr(65), "" + variables[f'theIdNumOfThe34theVar{variables["A_Index26"]}'] + "")
...
if (InStr(variables['cppCode'] , "FileCreateDir("))or(InStr(variables['cppCode'] , "FileCreateDir (")):
...
if (InStr(variables['cppCode'] , "FileRemoveDir("))or(InStr(variables['cppCode'] , "FileRemoveDir (")):
    variables['uperCodeLibs'] += "\n#include <filesystem>\n#include <system_error>\n"
...
if (InStr(variables['cppCode'] , "FileExist("))or(InStr(variables['cppCode'] , "FileExist (")):
    variables['uperCodeLibs'] += "\n#include <filesystem>\n#include <system_error>\n"
    variables['uperCode'] = variables['uperCode'] + "\n// Checks if a file or directory exists\nbool FileExist(const std::string& path) {\n    try {\n        return std::filesystem::exists(path);\n    } catch (const std::filesystem::filesystem_error&) {\n        // Handle errors silently; return false if an error occurs\n        return false;\n    }\n}\n"
if (InStr(variables['cppCode'] , "isWindows("))or(InStr(variables['cppCode'] , "isWindows (")):
    variables['uperCodeLibs'] += "\n#include <iostream>\n"
